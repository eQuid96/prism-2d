...
Shader::ShaderCompilationResult Shader::CompileShader(const std::string &src,
                                                      const Shader::Type type)
{
    unsigned int shaderId =
        glCreateShader(type == Shader::Type::VERTEX ? GL_VERTEX_SHADER : GL_FRAGMENT_SHADER);
    ...
    if (!result)
    {
        ...
        std::cout << "Error compiling "
                  << (type == Shader::Type::VERTEX ? "vertex shader" : "fragment shader")
                  << std::endl;
